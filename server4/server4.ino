...
RHRouter *manager;
// Простой счетчик - нужен для ответа сервера
int count = 0;
...
void setup() 
{
  ...
  manager = new RHRouter(driver, SERVER4_ADDRESS);
  // Инициализация
  ...
  manager->addRouteTo(CLIENT_ADDRESS,  SERVER3_ADDRESS);
  
  
  Serial.println("Нода SERVER " + (String) (SERVER4_ADDRESS-5) + ": работает, ок.");
  led_G();
}

uint8_t data[] = "Ответ от SERVER_4 - работает, ок.";
