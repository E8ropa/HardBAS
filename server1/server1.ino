...
void setup() 
{
  ...
  // Ручное определение маршрутов для этой сети
     ...
     manager->addRouteTo(SERVER4_ADDRESS, SERVER2_ADDRESS);
  
  Serial.println("Нода SERVER " + (String) (SERVER1_ADDRESS-2) + ": работает, ок.");
  led_G();
}

uint8_t data[] = "Ответ от SERVER_1 - работает, ок.";
...
void loop()
{
  ...
  { 
    ...
    // Отправляем ответ клиенту C1
    if (manager->sendtoWait(data, sizeof(data), from) != RH_ROUTER_ERROR_NONE){
      Serial.println("Ошибка sendtoWait.");
      led_R();
      }
  }
  if(manager->peekAtMessage())
}
